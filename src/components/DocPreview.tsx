...
export function DocPreview({
  ...
  return (
    <article
      id="doc-print-area"
      className="rounded-2xl border border-border bg-card p-8 shadow-sm print:rounded-none print:border-0 print:p-0 print:shadow-none"
    >
      <header className="flex flex-wrap items-start justify-between gap-6 border-b border-border pb-6">
        <div>
          <h1 className="text-2xl font-bold text-foreground">
            {doc.type === "invoice" ? "חשבונית מס" : "קבלה"}
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">מס׳ {doc.number}</p>
          <p className="mt-3 text-sm text-muted-foreground">
            תאריך הפקה: <span className="text-foreground">{dateHe(doc.issueDate)}</span>
          </p>
      ...
      <section className="border-b border-border py-5">
        ...
        {client?.taxId && (
          <p className="text-sm text-muted-foreground">ח.פ / ע.מ {client.taxId}</p>
        )}
        {client?.address && (
      ...
      <div className="mt-6 flex justify-end">
        <div className="w-full max-w-xs space-y-2 text-sm">
          ...
          <div className="flex justify-between rounded-lg bg-secondary px-3 py-2 text-base font-bold text-foreground">
            <span>סה״כ {typeLabel[doc.type]}</span>
