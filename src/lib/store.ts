...
function normalizeLegacy(raw: unknown): AppData {
  ...
  const business = legacy.business ?? { name: "", taxId: "", address: "", phone: "", email: "" };
  return { business: { ...business, businessType: business.businessType ?? "licensed", vatRate: business.vatRate ?? 18, documentPrefix: business.documentPrefix ?? "INV" }, clients: legacy.clients ?? [], docs: (legacy.docs ?? []).map((d) => ({ ...d, status: d.status === "draft" ? "draft" : d.status === "paid" ? "paid" : "issued", createdAt: d.createdAt ?? nowIso() })) as Doc[], audit: legacy.audit ?? [] };
}
...
function load(): AppData {
  ...
  try { const raw = window.localStorage.getItem(KEY) ?? window.localStorage.getItem(LEGACY_KEY); data = raw ? normalizeLegacy(JSON.parse(raw)) : seed(); } catch { data = seed(); }
  return data;
}
function commit(next: AppData) { data = next; if (typeof window !== "undefined") window.localStorage.setItem(KEY, JSON.stringify(next)); listeners.forEach((l) => l()); }
export function useData() { const [state, setState] = useState<AppData | null>(null); useEffect(() => { const sync = () => setState({ ...load() }); sync(); listeners.add(sync); return () => listeners.delete(sync); }, []); return state; }

export const actions = {
  saveBusiness(business: BusinessInfo) { commit({ ...load(), business }); },
  saveClient(client: Client) { const d = load(); const exists = d.clients.some((c) => c.id === client.id); commit({ ...d, clients: exists ? d.clients.map((c) => c.id === client.id ? client : c) : [...d.clients, client] }); },
  deleteClient(id: string) { const d = load(); commit({ ...d, clients: d.clients.filter((c) => c.id !== id) }); },
  ...
  setStatus(id: string, status: Exclude<DocStatus, "draft" | "cancelled">) { const d = load(); const doc = d.docs.find((x) => x.id === id); if (!doc || doc.status === "draft" || doc.status === "cancelled") return; const at = nowIso(); const hash = doc.documentHash ?? documentHash(doc); commit({ ...d, docs: d.docs.map((x) => x.id === id ? { ...x, status } : x), audit: [...d.audit, { id: uid(), docId: id, action: status, at, documentHash: hash }] }); },
  cancelDoc(id: string, reason: string) { const d = load(); const doc = d.docs.find((x) => x.id === id); if (!doc || doc.status === "draft" || doc.status === "cancelled") throw new Error("לא ניתן לבטל את המסמך במצב הנוכחי"); const at = nowIso(); const hash = doc.documentHash ?? documentHash(doc); const cancelled = { ...doc, status: "cancelled" as const, cancelledAt: at, cancellationReason: reason.trim() || "ביטול מסמך" }; commit({ ...d, docs: d.docs.map((x) => x.id === id ? cancelled : x), audit: [...d.audit, { id: uid(), docId: id, action: "cancelled", at, documentHash: hash, note: cancelled.cancellationReason }] }); },
