...
export const Route = createFileRoute("/_authenticated/clients")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "ניהול פרטי לקוחות ונתוני מס לשימוש בהפקת מסמכים." },
...
const blank = (): Client => ({ id: uid(), name: "", isVatRegistered: false });
        ...
        <section className="mt-5 rounded-2xl border border-border bg-card p-5 shadow-sm">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div>
              ...
              <label className="flex h-10 items-center gap-3 rounded-lg border border-input px-3 text-sm">
                <input
                  type="checkbox"
                  checked={draft.isVatRegistered}
                  onChange={(e) => setDraft({ ...draft, isVatRegistered: e.target.checked })}
                />
                הלקוח עוסק מורשה / רשום במע״מ
              </label>
            </div>
            <div>
